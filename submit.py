...
_LANGUAGE_GUESS = { '.java' : 'Java', '.c' : 'C', '.cpp' : 'C++', '.h' : 'C++', '.cc' : 'C++', '.cxx' : 'C++', '.c++' : 'C++', '.py' : 'Python 3', '.cs': 'C#', '.c#': 'C#', '.go': 'Go', '.m' : 'Objective-C', '.hs' : 'Haskell', '.pl' : 'Prolog', '.js': 'JavaScript', '.php': 'PHP' }
...
def submit(problem, language, files, force=True, mainclass=None, tag=None, username=None, password=None, token=None, debug=False):
	...
	try:
		ret = urllib2.urlopen(request).read().replace("<br />", "\n")
		# print ret
		if "Problem not found" in ret:
			sys.exit(2)
		if "Submission recieved" in ret:
			matchObj = re.match( r'Submission recieved. Submission ID: (\d+)\.', ret, re.M|re.I)
			if matchObj:
